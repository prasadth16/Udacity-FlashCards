...
class DeckInformation extends Component {
  ...
  newQuestionNavigation = (navigate, deckId) =>
    navigate('AddNewCard', { deckId: deckId });
  quizNavigation = (navigate, deckId, decks) =>
    navigate('Quiz', { DeckInformation: decks[deckId] });
  render() {
    ...
    if (Object.keys(decks).length > 0) {
      return (
        <View key={deckId} style={styles.container}>
          ...
          <View style={styles.row}>
            <NewQuestionBTN
              buttonCaption="New Question"
              deckId={deckId}
              navigate={this.props.navigation.navigate}
              onSubmit={this.newQuestionNavigation}
            />
            <QuizBTN
